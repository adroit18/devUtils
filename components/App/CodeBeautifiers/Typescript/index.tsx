...
export default function TypescriptFormatterBeautifier(): any {
  const [content, setContent] = React.useState(["", ""]);

  const getFormatted = React.useCallback((contentVal = []) => {
    const [inputVal] = contentVal;
    try {
      const inputTypescript = inputVal.toString();
      const outputTypescript = prettier.format(inputTypescript, {
        parser: "typescript",
        plugins: [parserTypescript],
      });
      setContent([inputVal, outputTypescript]);
    } catch (e) {
      setContent([inputVal, `invalid Typescript ERROR: ${e.message}`]);
    }
  }, []);
      ...
      <Grid container>
        ...
        <Grid item xs={12}>
          <SplitEditor
            ...
            orientation="beside"
            value={content}
            name="TYPESCRIPT_FORMAT_AND_VALIDATE"
